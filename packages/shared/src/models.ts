...
export interface WebhookSubscription {
  ...
  callbackUrl: string;
  /**
   * Per-subscription, client-supplied Standard Webhooks signing secret in
   * `whsec_` format (literal prefix `whsec_` followed by base64 of 24-64 random
   * bytes). Generated by the Subscription Manager and supplied to the MCP server
   * in `delivery.secret`; the server signs this subscription's deliveries with
   * it, and the Webhook Receiver looks it up by `subscriptionId` to verify.
   * Stored as a DynamoDB attribute, encrypted at rest via DynamoDB default
   * encryption.
   */
  secret: string;
