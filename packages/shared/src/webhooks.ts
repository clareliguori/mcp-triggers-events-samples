/**
 * Shared Standard Webhooks signing/verification helper used by BOTH the MCP
 * servers (sign) and the Webhook Receiver (verify).
 *
 * Standard Webhooks signing/verification (HMAC-SHA256 over
 * `{webhook-id}.{webhook-timestamp}.{payload}`, keyed by a symmetric `whsec_`
 * secret) is the scheme both sides of the system speak: the two MCP servers
 * sign each outbound delivery and the Webhook Receiver authenticates every
 * incoming one. Per the experimental MCP Events extension webhook delivery
 * mode, the signing secret is **per-subscription and client-supplied** — there
 * is no per-server secret. Each delivery therefore carries an
 * `X-MCP-Subscription-Id` header that selects which subscription's secret to
 * sign/verify against; the actual secret lookup/decrypt is wired up by the
 * callers (the MCP server delivery path and the Webhook Receiver handler) and
 * the resolved plaintext `whsec_` is passed to {@link signWebhook} /
 * {@link verifyWebhook} here.
 *
 ...
 * (Properties 1 and 2) and reused by every consumer:
 *
